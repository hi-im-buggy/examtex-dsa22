...
int comparator_fn(double A, double B, char comparator[2])
{
...
{
    Question_Bank * QB = (Question_Bank *) malloc(sizeof(Question_Bank) + (n * sizeof(Question)));
    QB->no_questions = n;
...
int get_Questions(User_Parameters * UP, Question_Bank * QB)
{
    ...
    for(int i = 0; i < QB->no_questions; i++)
    {
        ...
        if(comparator_fn(diffA, diffB, UP->comparator))
        {
            // increment the size of array to avoid overflow
            if(j == curr_size)
            {
                curr_size = curr_size + incr_size;
                Question_collection = (Question **) realloc(Question_collection, curr_size * sizeof(Question *));
            }
